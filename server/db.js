...
export function scoreLaunch(mint){
 ...
 const buyerVelocity = l.unique_buyers/(ageSec/60);
 const mcGrowth = l.last_market_cap_sol;
 let score=0;
 score += Math.min(35, buyerVelocity*7);
 score += Math.min(25, Math.min(buySellRatio,5)*5);
 score += Math.min(20, l.unique_buyers/2);
 score += Math.min(20, mcGrowth/5);
