...
onFeedOpen(() => {
  sendFeed({ method: "subscribeNewToken" });
  console.log("Screener: watching new pump.fun launches");
  for (const mint of watching) sendFeed({ method: "subscribeTokenTrade", keys: [mint] });
});
...
onFeedMessage(e => {
  if (e.txType === "create" && e.mint) {
    ...
    if (watching.size < MAX_WATCH) {
      watching.add(e.mint);
      sendFeed({ method: "subscribeTokenTrade", keys: [e.mint] });
      setTimeout(() => {
        watching.delete(e.mint);
        sendFeed({ method: "unsubscribeTokenTrade", keys: [e.mint] });
